"use client";

import { cn } from "@/core/utils/http.util";
import { normalizePurchaseOrderStatus } from "@/features/purchase-orders/utils/purchase-order-nested-fields.util";

type Props = {
  status: string | null | undefined;
  label: string;
  className?: string;
};

export function PurchaseOrderStatusBadge({ status, label, className }: Props) {
  const norm = normalizePurchaseOrderStatus(status);
  const tone =
    norm === "approved" || norm === "received"
      ? "bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-950/50 dark:text-emerald-300"
      : norm === "sent" || norm === "pending"
        ? "bg-amber-50 text-amber-800 ring-amber-600/20 dark:bg-amber-950/50 dark:text-amber-300"
        : norm === "cancelled"
          ? "bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-950/50 dark:text-red-300"
          : "bg-slate-100 text-slate-700 ring-slate-500/20 dark:bg-slate-800 dark:text-slate-300";

  return (
    <span
      className={cn(
        "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ring-1 ring-inset",
        tone,
        className,
      )}
    >
      {label}
    </span>
  );
}
