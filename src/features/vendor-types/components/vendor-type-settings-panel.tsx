  ...
  return (
    <div className="space-y-6">
      ...
      <DetailPanel
        ...
        title={detailRow ? <VendorTypeChip row={detailRow} className="text-base font-semibold" /> : null}
        subtitle={
          detailRow ? (
        ...
        {detailRow ? (
          <div className="space-y-5">
            <FieldGroup label={t("table.type")}>
              <p className="text-sm text-slate-800 dark:text-slate-200">{formatVendorTypeLabel(detailRow)}</p>
            </FieldGroup>
            ...
            <FieldGroup label={t("detail.createdAt")}>
              ...
              {vendorTypeUserLabel(detailRow.created_by) !== "—" ? (
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {t("detail.byUser", { user: vendorTypeUserLabel(detailRow.created_by) })}
                </p>
              ) : null}
            </FieldGroup>
            <FieldGroup label={t("detail.updatedAt")}>
              ...
              {vendorTypeUserLabel(detailRow.modified_by) !== "—" ? (
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {t("detail.byUser", { user: vendorTypeUserLabel(detailRow.modified_by) })}
                </p>
              ) : null}
            </FieldGroup>
          </div>
