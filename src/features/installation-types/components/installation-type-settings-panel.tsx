...
export function InstallationTypeSettingsPanel() {
  ...
  const countInactive = React.useCallback(async () => {
    const { pagination: p } = await fetchInstallationTypesPage(1, 1, {
      search: search || undefined,
      is_active: false,
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <DetailPanel
        ...
        title={detailRow ? <InstallationTypeChip row={detailRow} className="text-base font-semibold" /> : null}
        subtitle={
          detailRow ? (
        ...
        {detailRow ? (
          <div className="space-y-5">
            <FieldGroup label={t("table.type")}>
              <p className="text-sm text-slate-800 dark:text-slate-200">{formatInstallationTypeLabel(detailRow)}</p>
            </FieldGroup>
            ...
            <FieldGroup label={t("detail.createdAt")}>
              ...
              {installationTypeUserLabel(detailRow.created_by) !== "—" ? (
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {t("detail.byUser", { user: installationTypeUserLabel(detailRow.created_by) })}
                </p>
              ) : null}
            </FieldGroup>
            <FieldGroup label={t("detail.updatedAt")}>
              ...
              {installationTypeUserLabel(detailRow.modified_by) !== "—" ? (
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  {t("detail.byUser", { user: installationTypeUserLabel(detailRow.modified_by) })}
                </p>
              ) : null}
            </FieldGroup>
          </div>
