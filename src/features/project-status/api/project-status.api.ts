...
function normalizeProjectStatusRow(row: ProjectStatusApiRow): WorkflowColourStatus {
  return {
    id: row.id,
...
export async function fetchProjectStatusesPage(
  page = 1,
  pageSize = 20,
  filters?: ProjectStatusListFilters,
...
export async function fetchProjectStatus(id: number): Promise<WorkflowColourStatus> {
  const { data } = await api.get<ApiEnvelope<ProjectStatusApiRow>>(PROJECT_STATUS_PATHS.detail(id));
  assertApiSuccess(data);
  return normalizeProjectStatusRow(data.data);
}

export async function createProjectStatus(body: WorkflowColourStatusCreatePayload): Promise<WorkflowColourStatus> {
  const { data } = await api.post<ApiEnvelope<ProjectStatusApiRow>>(
    PROJECT_STATUS_PATHS.list,
    toWriteBody(body),
  );
  assertApiSuccess(data);
  return normalizeProjectStatusRow(data.data);
...
export async function updateProjectStatus(
  ...
  const { data } = await api.patch<ApiEnvelope<ProjectStatusApiRow>>(
    PROJECT_STATUS_PATHS.detail(id),
    toPatchBody(body),
  );
  assertApiSuccess(data);
  return normalizeProjectStatusRow(data.data);
...
export async function deleteProjectStatus(id: number): Promise<void> {
  const { data } = await api.delete<ApiEnvelope<unknown>>(PROJECT_STATUS_PATHS.detail(id));
  assertEnvelopeSuccess(data);
}
